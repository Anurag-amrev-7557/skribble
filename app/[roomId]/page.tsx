...
import { Brain, Palette, Sparkles, Clock, Check, Crown } from "lucide-react";
...
export default function RoomPage() {
    ...
    useEffect(() => {
        ...
        socket.on("timer-update", (time: number) => {
            setGameState((prev: any) => ({ ...prev, timeRemaining: time }));
        });
    ...
    return (
        <div className={`grid h-screen bg-background bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-100 via-background to-background dark:from-indigo-950/30 dark:via-background dark:to-background overflow-hidden font-sans text-foreground selection:bg-primary/20 
                        ...
                        .map((p: any, i: number) => (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                    ...
                                    {/* Host Crown */}
                                    {gameState.hostId === p.id && <div className="text-yellow-500" title="Room Host"><Crown size={16} fill="currentColor" /></div>}
                ...
                <div className="h-16 md:h-20 flex items-center justify-between px-2 md:px-8 z-20 shrink-0 border-b md:border-b-0 border-black/10 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md md:bg-transparent">
                    ...
                    <div className="flex items-center">
                        <div className="md:hidden flex flex-col items-center gap-0.5">
                            <div className="relative w-10 h-10 flex items-center justify-center">
                                ...
                                <span className={`absolute inset-0 flex items-center justify-center text-xs font-bold ${gameState.timeRemaining < 10 ? 'text-red-500' : 'text-foreground'}`}>{gameState.timeRemaining}</span>
                            </div>
                    ...
                    <div className="flex items-center">
                        ...
                        <div className="hidden md:flex bg-white/80 dark:bg-zinc-900/50 backdrop-blur-md border shadow-sm rounded-2xl p-2 pr-4 items-center gap-3">
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold shadow-inner ${gameState.timeRemaining < 10 ? "bg-red-500 text-white animate-pulse" : "bg-zinc-100 dark:bg-zinc-800 text-foreground"}`}>{gameState.timeRemaining}</div>
                            ...
                            <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center text-white z-50 animate-in fade-in duration-300 p-4 text-center">
                                {isDrawer ? (
                                    <>
                                        ...
                                        <div className="mt-8 text-white/50 animate-pulse text-sm font-mono">Auto-selecting in {gameState.timeRemaining}s...</div>
                                    </>
                                ) : (
                                    <>
            ...
            <div className={`
                ...
                md:hidden
                w-full bg-background border-t z-50`}>
                <form onSubmit={(e) => {
                    e.preventDefault();
                    ...
                    <input name="mobile-guess" type="text" placeholder="Type your guess here..." className="flex-1 h-12 px-4 bg-muted/50 focus:outline-none focus:ring-2 focus:ring-primary shadow-sm" autoComplete="off" />
                    <Button type="submit" size="icon" className="w-12 h-12 rounded-[0px]">
